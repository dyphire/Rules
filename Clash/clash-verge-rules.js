...
function main(params) {
  ...
  // 香港地区
  const hongKongRegex = /HK|香港|港|HONG|Hong|HGC/;
  const hongKongProxies = params.proxies
    .filter((e) => hongKongRegex.test(e.name))
    .map((e) => e.name);
  // 台湾地区
  const taiwanRegex = /TW|台湾|台|Taiwan|新北|彰化|Tai/;
  const taiwanProxies = params.proxies
    .filter((e) => taiwanRegex.test(e.name))
    .map((e) => e.name);
  // 狮城地区
  const singaporeRegex = /SG|新加坡|坡|Singapore|狮城|Singa/;
  const singaporeProxies = params.proxies
    .filter((e) => singaporeRegex.test(e.name))
    .map((e) => e.name);
  // 日本地区
  const japanRegex = /JP|Jap|日本|日|Japan|川日|东京|大阪|泉日|埼玉|沪日|深日/;
  const japanProxies = params.proxies
    .filter((e) => japanRegex.test(e.name))
    .map((e) => e.name);
  // 美国地区
  const americaRegex = /US|美国|美|United States|波特兰|达拉斯|俄勒冈|凤凰城|费利蒙|硅谷|拉斯维加斯|洛杉矶|圣何塞|圣克拉拉|西雅图|芝加哥/;
